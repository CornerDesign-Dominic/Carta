...
function selfReceiptState(): SelfReceiptEditorState { const business = data({ selfReceiptId: 'EB-1', receiptDate: '2026-07-24', expenseDate: '2026-07-23' }, { internalReference: 'I', externalReference: 'E', costCenter: 'K-1' }); return { labels: labels(selfReceiptLabelKeys), selfReceiptData: { sender: business.sender, recipient: business.recipient, details: { selfReceiptId: 'EB-1', receiptDate: '2026-07-24', expenseDate: '2026-07-23' }, references: { internalReference: 'I', externalReference: 'E', costCenter: 'K-1' }, footer: business.footer, expenseInfo: { occasion: 'Termin', reason: 'Kein Fremdbeleg', settlementType: 'Bar', location: 'Berlin' } }, positions: [{ id: '123e4567-e89b-42d3-a456-426614174002', description: 'Leistung', netAmount: '42,00', taxRate: '19' }], fieldConfig: { ...businessConfig(['selfReceiptId', 'receiptDate', 'expenseDate', 'internalReference', 'externalReference']), expenseInfo: { hidden: ['reason'], order: ['reason', 'settlementType', 'location'] }, signature: { hidden: ['signature'], order: ['signature'] } } }; }
function shortSelfReceiptState(): SelfReceiptEditorState { return { ...selfReceiptState(), selfReceiptVariant: 'short', shortSelfReceipt: { title: 'Eigenbeleg Barzahlung', receiptNumber: 'Beleg Nr. 001', expenseDate: '2026-07-24', lineLabels: { receiptNumber: 'Belegnummer', reason: 'Grund', expenseDate: 'Tag der Ausgabe', purpose: 'Ausgabe für', location: 'Zahlungsempfänger\n(Name & Adresse)' }, recipientAddress: 'Firma Meier\nMusterstraße 12\n12345 Musterhausen', purpose: 'Bürostuhl', reason: 'Originalbeleg verloren', location: 'Musterhaus Berlin', date: '2026-07-24', dateLabel: 'Vermerk', signatureLabel: 'Freigabe', signatureValue: 'Max Mustermann', ownAddress: { company: 'Muster GmbH', street: 'Musterstraße 1', cityLine: '10115 Berlin' }, amount: { calculationSource: 'netAmount', sourceAmount: '100,00', taxRate: '19' }, fieldConfig: { note: { hidden: ['note'], order: ['note'] }, signature: { hidden: ['signature'], order: ['signature'] }, header: { hidden: ['streetLine'], order: ['company', 'streetLine', 'cityLine'] } } } }; }
...
describe('offer, delivery note and receipt PDF imports', () => {
  ...
  it('roundtrips the validated short self receipt variant and its visible-state configuration', async () => {
    ...
    expect(document.documentData.state.shortSelfReceipt).toMatchObject({ dateLabel: 'Vermerk', signatureLabel: 'Freigabe', signatureValue: 'Max Mustermann', ownAddress: { company: 'Muster GmbH', street: 'Musterstraße 1', cityLine: '10115 Berlin' } });
    expect(restoreSelfReceiptState(document)).toEqual({ status: 'valid', state });
    await expect(importSelfReceiptPdf(await embedBelege24DocumentInPdf(await plainPdf(), document))).resolves.toEqual({ status: 'valid', state, message: 'PDF erfolgreich geladen.' });
  });
