...
export default function CatalogItemMasterDataEditor() {
  ...
  return <div className="partner-editor">
    ...
    {hasStartedCollection && (draft ? <section className="partner-editor-section catalog-item-editor-section" aria-label="Eintrag bearbeiten"><CatalogItemForm item={draft} entryStatus={!draftSourceId ? 'new' : isDraftDirty ? 'edited' : 'saved'} titleInputRef={titleInputRef} onUpdateField={updateDraft} actions={<><div className="catalog-item-form-save-actions"><button className="partner-button is-primary" type="button" onClick={handleSaveDraft}>Übernehmen</button><button className="partner-button" type="button" onClick={handleSaveAndCreate}>Übernehmen & neuer Eintrag</button>{canDiscardDraft && <button className="partner-button" type="button" onClick={handleDiscardDraft}>{draftSourceId ? 'Änderungen verwerfen' : 'Entwurf verwerfen'}</button>}</div>{draftSourceId && <span className="catalog-item-form-delete-action"><button className="partner-button" type="button" onClick={handleDeleteDraft}>Löschen</button></span>}</>} /></section> : <section className="partner-editor-section catalog-new-entry-empty"><button className="partner-button is-primary" type="button" onClick={handleCreate}>Eintrag anlegen</button></section>)}
