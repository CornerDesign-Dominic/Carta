  ...
  return <form className="partner-form own-data-form" onSubmit={(event) => event.preventDefault()}>
    ...
    <FormArea title="Angaben für die Schnellauswahl in Belege24-Dokumenten" hint="Diese Angaben können später schnell in z. B. Rechnungen, Angebote oder Lieferscheine übernommen werden.">
      <FormSection title="Anschrift"><div className="own-data-grid own-data-address-grid">
        <Field inputRef={companyInputRef} id={`own-data-${record.id}-address-company-name`} label="Unternehmensname inkl. Rechtsform und Zusatz" className="own-data-span-two" value={record.address.companyName} onChange={(event) => onUpdateField(['address', 'companyName'], event.target.value)} />
      ...
      <FormSection title="Kontaktdaten"><div className="own-data-grid">{field(['contact', 'email'], 'E-Mail', '', 'email')}{field(['contact', 'phone'], 'Telefon', '', 'tel')}{field(['contact', 'fax'], 'Fax', '', 'tel')}{field(['contact', 'website'], 'Website', '', 'url')}</div></FormSection>
      ...
      <FormSection title="Steuer- und Registerdaten"><div className="own-data-grid">{field(['taxAndRegister', 'taxNumber'], 'Steuernummer')}{field(['taxAndRegister', 'vatId'], 'USt-IdNr.')}<Field id={`own-data-${record.id}-tax-and-register-register-entry`} label="Handelsregistereintrag" className="own-data-span-two" value={registerEntryValue} onChange={(event) => { onUpdateField(['taxAndRegister', 'commercialRegister'], event.target.value); onUpdateField(['taxAndRegister', 'registerNumber'], ''); }} />{field(['taxAndRegister', 'registerCourt'], 'Registergericht')}</div></FormSection>
      <FormSection title="Weiteres"><div className="own-data-grid own-data-two-grid">{field(['ownerOrManagingDirector'], 'Inhaber / Geschäftsführer')}{field(['documentHeaderName'], 'Titel / Name / Markenname')}<label className="partner-checkbox-field"><input type="checkbox" checked={record.settings.isSmallBusiness} onChange={(event) => onUpdateField(['settings', 'isSmallBusiness'], event.target.checked)} /><span>Kleinunternehmerregelung</span></label><Field id={`own-data-${record.id}-payment-term`} label="Standard-Zahlungsziel in Tagen" type="number" min="1" step="1" inputMode="numeric" value={record.settings.defaultPaymentTermDays} onChange={(event) => onUpdateField(['settings', 'defaultPaymentTermDays'], event.target.value.replace(/\D/g, ''))} /></div></FormSection>
    </FormArea>
