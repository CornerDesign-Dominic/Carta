...
export default function OwnDataForm({ record, entryStatus, companyInputRef, onUpdateField, actions }) {
  ...
  return <form className="partner-form own-data-form" onSubmit={(event) => event.preventDefault()}>
    ...
    <FormArea title="Angaben für die Schnellauswahl in Belege24-Dokumenten" hint="Diese Angaben können später schnell in z. B. Rechnungen, Angebote oder Lieferscheine übernommen werden.">
      ...
      <FormSection title="Steuer- und Registerdaten"><div className="own-data-grid">{field(['taxAndRegister', 'taxNumber'], 'Steuernummer')}{field(['taxAndRegister', 'vatId'], 'USt-IdNr.')}{field(['taxAndRegister', 'commercialRegister'], 'Handelsregister / Registereintrag')}{field(['taxAndRegister', 'registerNumber'], 'Registernummer')}{field(['taxAndRegister', 'registerCourt'], 'Registergericht')}</div></FormSection>
