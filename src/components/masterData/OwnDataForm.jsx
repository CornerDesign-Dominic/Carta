...
export default function OwnDataForm({ record, entryStatus, companyInputRef, onUpdateField, actions }) {
  ...
  return <form className="partner-form own-data-form" onSubmit={(event) => event.preventDefault()}>
    ...
    <FormArea title="Angaben für die Schnellauswahl in Belege24-Dokumenten" hint="Diese Angaben können später schnell in z. B. Rechnungen, Angebote oder Lieferscheine übernommen werden.">
      ...
      <FormSection title="Steuer- und Registerdaten"><div className="own-data-grid own-data-two-grid">{field(['taxAndRegister', 'taxNumber'], 'Steuernummer')}{field(['taxAndRegister', 'vatId'], 'USt-IdNr.')}<Field id={`own-data-${record.id}-tax-and-register-register-entry`} label="Handelsregistereintrag" value={registerEntryValue} onChange={(event) => { onUpdateField(['taxAndRegister', 'commercialRegister'], event.target.value); onUpdateField(['taxAndRegister', 'registerNumber'], ''); }} />{field(['taxAndRegister', 'registerCourt'], 'Registergericht')}</div></FormSection>
