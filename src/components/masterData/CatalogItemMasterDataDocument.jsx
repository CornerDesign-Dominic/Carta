...
export default function CatalogItemMasterDataDocument({ records, pagesRef, toolbar }) {
  ...
  return <section className="partner-document-preview catalog-item-document-preview" aria-labelledby="catalog-document-preview-title"><div className="partner-document-preview-heading"><h2 id="catalog-document-preview-title">Dokumentvorschau</h2></div>{toolbar}<div className="partner-document-pages" ref={pagesRef}>
    {records.map((item, index) => <article className="partner-document-page" aria-label={`Leistungs- und Artikeldatenblatt ${index + 1}`} key={item.id}><header className="partner-document-page-header"><strong>Leistungs- und Artikelstammdaten</strong><span>Belege24</span></header><div className="partner-document-page-meta"><span>Erstellungsdatum: {dateLabel}</span></div><ItemBlock item={item} index={index} /><footer className="partner-document-page-footer">Seite {index + 1}</footer></article>)}
    {!records.length && <article className="partner-document-page" aria-label="Leere Leistungs- und Artikelstammdatensammlung"><header className="partner-document-page-header"><strong>Leistungs- und Artikelstammdaten</strong><span>Belege24</span></header><div className="partner-document-page-meta"><span>Erstellungsdatum: {dateLabel}</span></div><p className="partner-document-empty">Noch keine Daten übernommen oder Stammdaten geladen.</p><footer className="partner-document-page-footer">Seite 1</footer></article>}
  </div></section>;
}
