...
function TextareaField({ label, helpText, ...props }) {
  ...
  return <label className="partner-field partner-field-wide">{label && <span>{label}{helpText && <FieldHelpTooltip label={label}>{helpText}</FieldHelpTooltip>}</span>}<textarea {...props} ref={textareaRef} rows="2" onInput={(event) => { resizeTextarea(event.currentTarget); props.onInput?.(event); }} /></label>;
}
...
function DeliveryAddressFields({ address, idPrefix, onChange }) {
  ...
  return <div className="partner-master-grid partner-master-two-grid delivery-address-fields">
    ...
    {field('attention', 'Adresszusatz / z. Hd.')}
...
export default function PartnerForm({
  ...
  return <form className="partner-form partner-master-data-form" onSubmit={(event) => event.preventDefault()}>
    ...
    <FormArea title="Daten für Belege24-Dokumente" hint="Diese Angaben können später schnell in z. B. Rechnungen, Angebote oder Lieferscheine übernommen werden.">
      <FormSection title="Anschrift">
        <div className="partner-master-grid partner-master-partner-grid">
          ...
          <Field id="partner-main-address-attention" label="Adresszusatz / z. Hd." value={partner.mainAddress.attention} onChange={(event) => update(['mainAddress', 'attention'], event.target.value)} />
      ...
      <FormSection title="Kontaktdaten"><div className="partner-master-grid partner-master-two-grid"><Field id="partner-email" label="E-Mail" type="email" value={partner.contact.email} onChange={(event) => update(['contact', 'email'], event.target.value)} /><Field id="partner-phone" label="Telefon" type="tel" value={partner.contact.phone} onChange={(event) => update(['contact', 'phone'], event.target.value)} /></div></FormSection>
      <FormSection title="Steuerdaten"><div className="partner-master-grid partner-master-two-grid"><Field id="partner-vat-id" label="Umsatzsteuer-ID" value={partner.tax.vatId} onChange={(event) => update(['tax', 'vatId'], event.target.value)} /><Field id="partner-tax-number" label="Steuernummer" value={partner.tax.taxNumber} onChange={(event) => update(['tax', 'taxNumber'], event.target.value)} /></div></FormSection>
      ...
      <FormSection title="Lieferanschriften" helpText="Können bei Warenrechnungen und Lieferscheinen unabhängig von der Hauptanschrift ausgewählt werden.">
        <p className="partner-form-help">Lieferanschriften können später unabhängig von der Hauptanschrift ausgewählt werden.</p>
        ...
        <button className="partner-button" type="button" style={{ borderColor: 'var(--color-primary)', color: 'var(--color-primary)', background: 'var(--color-card)', marginBottom: '5px' }} onMouseEnter={(event) => { event.currentTarget.style.borderColor = 'var(--color-primary-hover)'; event.currentTarget.style.color = 'var(--color-primary-hover)'; event.currentTarget.style.background = 'var(--color-accent-surface)'; }} onMouseLeave={(event) => { event.currentTarget.style.borderColor = 'var(--color-primary)'; event.currentTarget.style.color = 'var(--color-primary)'; event.currentTarget.style.background = 'var(--color-card)'; }} onClick={handleAddDeliveryAddress}>Lieferanschrift hinzufügen</button>
      </FormSection>
    </FormArea>
    <FormArea internal title="Angaben für die Stammdatenverwaltung" hint="Diese Angaben dienen ausschließlich der Verwaltung und werden in kein Dokument übernommen.">
      ...
      <FormSection title="Interne Notiz" helpText="Bleibt ausschließlich in der Stammdatenverwaltung und wird nicht in Dokumente übernommen."><TextareaField id="partner-notes-text" value={partner.notes} onChange={(event) => update(['notes'], event.target.value)} /></FormSection>
    </FormArea>
