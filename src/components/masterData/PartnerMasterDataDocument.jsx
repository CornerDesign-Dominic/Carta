...
function DetailSection({ title, children }) {
  ...
  return items.length ? <section className="partner-document-section"><h3>{title}</h3><div>{items}</div></section> : null;
}
function AddressLines({ address }) {
  return [address.companyName, address.attention, address.department, joinLine(address.street, address.houseNumber), joinLine(address.postalCode, address.city), address.country].filter(Boolean).map((line, index) => <p key={`${line}-${index}`}>{line}</p>);
}
...
function PartnerDocumentBlock({ partner, index }) {
  ...
  return <article className="partner-document-block">
    <header className="partner-document-block-header"><div><h2><span className="partner-document-index">{String(index + 1).padStart(2, '0')}</span> {getPartnerDisplayName(partner)}</h2><p>{getPartnerTypeLabel(partner.type)} · {partner.isActive ? 'Aktiv' : 'Inaktiv'}</p></div>{partner.legalForm && <span>{partner.legalForm}</span>}</header>
    {hasMainAddress && <DetailSection title="Hauptanschrift"><div className="partner-document-address"><AddressLines address={partner.mainAddress} /></div></DetailSection>}
...
export default function PartnerMasterDataDocument({ partners, pagesRef, toolbar }) {
  ...
  return <section className="partner-document-preview" aria-labelledby="partner-document-preview-title">
    ...
    <div className="partner-document-pages" ref={pagesRef}>
      {partners.map((partner, index) => <article className="partner-document-page" aria-label={`Stammdatenblatt ${index + 1}`} key={partner.id}>
        <header className="partner-document-page-header"><strong>Belege24</strong><span>Stammdatensammlung</span></header>
        <div className="partner-document-page-meta"><span>Anzahl Partner: {partners.length}</span><span>Stand: {dateLabel}</span></div>
      ...
      {!partners.length && <article className="partner-document-page" aria-label="Leeres Stammdatenblatt">
        <header className="partner-document-page-header"><strong>Belege24</strong><span>Stammdatensammlung</span></header>
        <div className="partner-document-page-meta"><span>Anzahl Partner: 0</span><span>Stand: {dateLabel}</span></div>
        <h1 className="partner-document-title">Partner- und Empfängerstammdaten</h1>
        <p className="partner-document-empty">Noch keine Partner gespeichert.</p>
        <footer className="partner-document-page-footer">Seite 1</footer>
      </article>}
    </div>
  </section>;
}
