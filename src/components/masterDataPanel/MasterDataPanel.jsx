...
export function OwnDataPanel({ appliedOwnData, documentAdapter, lastUsedOwnData, records, selectedId, onApply, onRemove, onSelect }) {
  ...
  })}</div></>}<div className="master-data-panel-actions"><button className="partner-button is-primary" type="button" disabled={!selectedRecord || !documentAdapter || isSelectedApplied} onClick={() => onApply()}>Auswahl übernehmen</button>{appliedOwnData && <button className="partner-button" type="button" onClick={onRemove}>Aus Dokument entfernen</button>}</div>{appliedOwnData?.sourceFileRemoved && <p className="master-data-panel-note">Die Quelldatei wurde aus dem Panel entfernt. Die bereits übernommenen Daten bleiben im Dokument erhalten.</p>}</div>;
}
